...
//			System.out.println(location);
	...
	public ResponseEntity<Object> replaceGreetingByParameter(@PathVariable("language") String language,
			@PathVariable("greeting") String greeting) {
		Greeting greetingObject = new Greeting(language, greeting);
		if (repo.contains(language)) {
			if (repo.contains(greetingObject)) {
				return new ResponseEntity<>("Exact resource already exists!", HttpStatus.CONFLICT); // No update because
																									// the
																									// exact resource
																									// exists!.
			} else {
				if (repo.replaceGreeting(greetingObject)) {
					...
					return new ResponseEntity<>(HttpHeaders.CONTENT_TYPE, HttpStatus.OK);
				} else
					return new ResponseEntity<>("A problem occurred during replacement!", HttpStatus.CONFLICT);
			}
		} else { // Just add it as a new resource!
			repo.addGreeting(greetingObject);
			URI location = ServletUriComponentsBuilder.fromCurrentRequest()
					.replacePath(greetingObject.getLanguage() + "/" + greetingObject.getLanguage()).build().toUri();
			return ResponseEntity.created(location).build();
		}
	}
